...
10 across two experimental conditions and one input sample.
For the subsequent analysis, we'll instead use a pre-prepared set of windows, 
however this code serves as an excellent example of how to structure your code.
It should also be noted that counting all reads across a `BamFileList` using 
sliding windows, **will require a significant amount of RAM** and will be 
...
We can check which windows had more than one read by checking the ranges 
...
Now that the counting filtering steps have been demonstrated, we'll use an 
object containing H3K27ac signal for 6 samples, across two treatments, but 
restricted to a subset of Chromosome 10.
This data was prepared using the above steps and contains reads from the 
MDA-MB-453 breast cancer cell line, under Vehicle and DHT-treated conditions.
The original samples are included in the GEO dataset `GSE176010`
...
As logCPM transformed values can be helpful, let's add this assay, then look at our densities once again.

```{r}
assay(h3k_chr10, "logCPM") <- cpm(
    assay(h3k_chr10, "counts"), lib.size = h3k_chr10$totals, log = TRUE
)
plotAssayDensities(h3k_chr10, assay = "logCPM", colour = "treat") +
  scale_colour_brewer(palette = "Set1") +
  theme_bw()
```

We can also perform some basic QC using RLE plots.
...
```{r plot-assay-rle, fig.cap = "RLE plots across all samples (A) and with values calculated within treatment groups (B)."}
a <- plotAssayRle(h3k_chr10, assay = "logCPM", fill = "treat") +
  geom_hline(yintercept = 0, linetype = 2, colour = "grey") +
  scale_fill_brewer(palette = "Set1") +
  theme_bw()
b <- plotAssayRle(
    h3k_chr10, assay = "logCPM", fill = "treat", rle_group = "treat"
) +
  geom_hline(yintercept = 0, linetype = 2, colour = "grey") +
  scale_fill_brewer(palette = "Set1") +
  theme_bw()
...
```{r plot-pca}
plotAssayPCA(h3k_chr10, "logCPM", colour = "treat", label = "label") +
  scale_colour_brewer(palette = "Set1") +
  theme_bw()
```
